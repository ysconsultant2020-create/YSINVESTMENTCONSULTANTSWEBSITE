...
const LumpsumCalculatorPage = () => {
  ...
  const result = useMemo(() => {
    ...
    return { totalInvested: Math.round(amount), returns: Math.round(returns), futureValue: Math.round(futureValue) };
  }, [amount, rate, years]);
      ...
      <div className="pt-24 pb-12 bg-gradient-hero">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-display text-white mb-4">Lumpsum Calculator</h1>
            <p className="text-white/60 text-lg max-w-xl mx-auto">Calculate returns on one-time investments</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-8">
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-white/60 text-sm">Investment Amount</label>
                  <span className="text-gold-400 font-semibold">{fmt(amount)}</span>
                </div>
                ...
                <div className="flex justify-between mt-1 text-xs text-white/30"><span>₹10,000</span><span>₹1 Cr</span></div>
              </div>
              <div>
                <div className="flex justify-between mb-2">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-8">
            ...
            <div className="h-64 mb-8"><Doughnut data={chartData} options={chartOptions} /></div>
